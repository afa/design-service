...
namespace :questionarie do
	...
	task :init => :environment do
		...
		questionnaire = Questionnaire.new(:name => "Архитекторы", :specialization_id => 7, :is_numeric => true)
		...
		question6 = Question.new(
			:position => 3,
			:name => 'Совмещаете специальность дизайнера?',
			:count_true => 0,
			:count_all => 0,
			:kind => 'yes_no',
			:is_necessarily => false,
		...
		question6.question_fields << QuestionField.new(
				:name => '',
				:value => '',
				:display => 'yes_no',
				:position => 1,
